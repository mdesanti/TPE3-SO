...
static priorityNode * insertElemR(priorityQueue * pq, process * proc, int priority, priorityNode * actual);
...
priorityNode * insertElemR(priorityQueue * pq, process * proc, int priority, priorityNode * actual) {
	...
	if(actual == NULL || actual->priority > priority) {
		...
		actual->next = (struct priorityNode*)insertElemR(pq, proc, priority, (priorityNode*)actual->next);
		return actual;
	}
}
